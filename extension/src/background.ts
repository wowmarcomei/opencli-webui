...
import { DEFAULT_REMOTE_BRIDGE_URL, REMOTE_BRIDGE_PATH, WS_RECONNECT_BASE_DELAY, WS_RECONNECT_MAX_DELAY } from './protocol';
...
const BRIDGE_URL_KEY = 'opencli_remote_bridge_url_v1';
const BRIDGE_TOKEN_KEY = 'opencli_remote_bridge_token_v1';
...
function normalizeBridgeUrl(value: unknown): string {
  const raw = typeof value === 'string' && value.trim() ? value.trim() : DEFAULT_REMOTE_BRIDGE_URL;
  return raw.replace(/\/+$/, '');
}

async function getBridgeSettings(): Promise<BridgeSettings> {
  try {
    const local = chrome.storage?.local;
    if (!local) return { baseUrl: DEFAULT_REMOTE_BRIDGE_URL, token: '' };
    const raw = await local.get([BRIDGE_URL_KEY, BRIDGE_TOKEN_KEY]) as Record<string, unknown>;
    return {
      baseUrl: normalizeBridgeUrl(raw[BRIDGE_URL_KEY]),
      token: typeof raw[BRIDGE_TOKEN_KEY] === 'string' ? raw[BRIDGE_TOKEN_KEY].trim() : '',
    };
  } catch {
    return { baseUrl: DEFAULT_REMOTE_BRIDGE_URL, token: '' };
  }
}

function bridgeUrl(settings: BridgeSettings, endpoint: 'ping' | 'ws'): string {
  const url = new URL(`${REMOTE_BRIDGE_PATH}/${endpoint}`, settings.baseUrl);
...
async function saveBridgeSettings(baseUrl: string, token: string): Promise<BridgeSettings> {
  const settings = {
    baseUrl: normalizeBridgeUrl(baseUrl),
    token: token.trim(),
  };
  await chrome.storage?.local?.set({
    [BRIDGE_URL_KEY]: settings.baseUrl,
    [BRIDGE_TOKEN_KEY]: settings.token,
  });
  ws?.close();
  ws = null;
  reconnectAttempts = 0;
...
async function connect(): Promise<void> {
  ...
  try {
    const res = await fetch(bridgeUrl(settings, 'ping'), { signal: AbortSignal.timeout(1000) });
  ...
  try {
    ...
    ws = new WebSocket(bridgeUrl(settings, 'ws'));
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg?.type === 'saveSettings') {
    void (async () => {
      const settings = await saveBridgeSettings(String(msg.baseUrl ?? ''), String(msg.token ?? ''));
      sendResponse({ ok: true, bridgeUrl: settings.baseUrl, tokenConfigured: settings.token.length > 0 });
    })();
    return true;
  }
...
async function fetchDaemonVersion(): Promise<string | null> {
  try {
    const settings = await getBridgeSettings();
    const res = await fetch(bridgeUrl(settings, 'ping'), {
      method: 'GET',
